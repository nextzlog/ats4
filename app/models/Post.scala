...
object Post {
	...
	def ofSect(sect: String)(implicit db: Database) = db.withConnection{implicit conn =>
		SQL("select * from post where sect like '{sect}%' order by id").on('sect->sect).as(parser.*)
	}.sortBy(_.score).reverse
	def ofCall(call: String)(implicit db: Database) = db.withConnection{implicit conn =>
		SQL("select * from post where call like '{call}%' order by id").on('call->call).as(parser.*)
	}.sortBy(_.call)
}
